...
const ContactForm = () => {
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!formRef.current.username.value || !formRef.current.email.value || !formRef.current.subject.value || !formRef.current.message.value) {
    ...
    return (
        <form ref={formRef} onSubmit={handleSubmit} className='rounded-lg bg-[var(--color-bg-accent)] p-4 sm:p-8 grid grid-cols-2 grid-rows-[auto_auto_auto_auto_1fr_auto] sm:grid-rows-[auto_auto_auto_1fr_auto] gap-x-10 gap-y-5 w-full h-full'>
            ...
            <input type='text' name="username" placeholder="Your Name" className="p-2 bg-[var(--color-background)] rounded-lg w-full col-span-2 sm:col-span-1"/>
